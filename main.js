    ...
    init() {
        ...
        scene.addEventListener('click', (e) => {
            ...
            if (!this.spawned) {
                ...
                // 【修正】出現位置を 6m 前方に。
                // 身長が高いため、このくらい離さないとカメラの「死角（下側）」に入り込みます
                const dist = 6.0;
                ...
                // 青いグリッド床もずんだもんの足元（-3.0m）に同期
                const floor = document.querySelector('#dummy-floor');
                if (floor) floor.setAttribute('position', `0 ${y} 0`);

                console.log("Spawn point fixed at Y: -3.0, Dist: 6.0");
            }
        });
    }
});